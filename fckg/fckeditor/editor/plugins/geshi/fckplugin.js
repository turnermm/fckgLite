...
FCKGeshi.Insert = function(code_type, lang, snip_fname, isSafari, strip_html) {    
    ...
    mySelection = mySelection.replace(/\s+$/,"");
    if (strip_html == "re") {
        mySelection = this.StripHtmlRe(mySelection);        
    } else if (strip_html == "dom") {
        mySelection = this.StripHtmlDom(mySelection);
    }
...
/* Strip html tags with preserving whitespaces and assuming that br, p and div introduce new line */
FCKGeshi.StripHtmlRe = function(htmlText) {
    var result = htmlText;    
